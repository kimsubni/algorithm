    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < Integer.parseInt(info[1]) + Integer.parseInt(info[2]); i++) {
            ...
            if (Integer.parseInt(operation[0]) == 1) {
                ...
                st.update2(1, 1, Integer.parseInt(info[0]), Integer.parseInt(operation[1]),
                        Long.parseLong(operation[2]));
            } else {
                // 배열의 특정 구간 합 구하기
                long result = st.sum(1, 1, Integer.parseInt(info[0]), Integer.parseInt(operation[1]),
